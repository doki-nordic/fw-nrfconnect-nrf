...
def load_license_texts():
    ...
    for a in license_texts:
        for b in license_texts:
            pass#if (a[0] != b[0]) and (a[1].find(b[1]) >= 0):
                # raise Exception(f'License text for {b[0]} is part of {a[0]}. '
                #     'This situation is not implemented. Implement it before adding to '
                #     'licenses.yaml file.')
                #TODO: maybe following solution during detecting:
                # detect all licenses
                # if there are more liceses and they are ovelapping:
                #     select bigger one
                #     add to result set
                #     remove from the text
                #     recusivly detect licenses from the modified text and add to result set
    file_types = []
