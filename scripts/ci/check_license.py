...
class PatchLicenseChecker:
    ...
    def report(self, label: str, message: str, file_name:'str|Path' = '<none>', license:str = ''):
        ...
        if license == '':
            ...
            license_stdout = f'{license} - '
            license_xml = f':{html.escape(license)}'
            license_github = f',title={urllib.parse.quote(str(license))}'
        # Print to stdout with optional GitHub Actions Workflow commands.
        if label not in ('error', 'warning'):
            print(f'{label.upper()}: {file_name}: {license_stdout}{message}')
        else:
            if self.args.github:
                file_path = (self.west_workspace / file_name).relative_to(self.git_top)
                print(f'::{label} '
                      f'file={urllib.parse.quote(str(file_path))}{license_github}::'
                      f'{urllib.parse.quote(message)}')
            else:
                print(f'{label.upper()}: {file_name}: {license_stdout}{message}', file=sys.stderr)
    ...
    def skip_files(self, files:'list[Path]') -> 'list[Path]':
        ...
        for file_name in files:
            if not (self.west_workspace / file_name).exists():
                self.report('skip', 'file does not exist', file_name)
            if not (self.west_workspace / file_name).is_file():
                self.report('skip', 'it is not a file', file_name)
            elif self.license_checker.check('ANY', file_name):
                self.report('skip', 'any license allowed', file_name)
            else:
        ...
        for file_name, file_info in detected.items():
            ...
            for license in file_info['license']:
                if self.license_checker.check(license, file_name):
                    self.report('ok', 'valid', file_name, license)
                elif self.license_checker.check('-' + license, file_name):
                    self.report('warning', 'valid with a warning', file_name, license)
                elif self.license_checker.check('-ANY', file_name):
                    self.report('warning', 'any license allowed with a warning', file_name, license)
                else:
                    self.report('error', 'invalid', file_name, license)
